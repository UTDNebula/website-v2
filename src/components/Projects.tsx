...
export default function Projects() {
  ...
  return (
    <div className="h-[1623px] shrink-0 flex flex-col justify-center ">
      <Image src={galaxy_circle} alt={''} priority className="absolute -z-20 h-auto w-full" />
  ...
  return (
    <div className="w-[1172px] h-[531px] rounded-3xl shrink-0 border border-white text-white">
      <div className="flex w-90 h-96 flex-col items-start gap-8 flex-shrink-0">
        <h1>
          <Image src={planner_card} alt={''} priority className="absolute h-auto w-auto" />
        </h1>
        <h1 className="font-kallisto text-3xl">{currentProj.title}</h1>
        <p className=" w-80 h-40 flex-shrink-0 font-inter text-lg">{currentProj.description}</p>
        ...
        <div className="flex p-3 justify-center items-center gap-10 ">
          ...
          <p className="text-lg font-semibold">{props.index + 1} / 6 </p>
          <button className="flex p-3 items-start gap-3 rounded-full bg-white">
            <Image
              src="/arrow.svg"
              alt="arrow"
