...
const governance = [
  {
    name: 'Officers',
    description: 'As a group of leaders, our officer team works together to guide our organization towards its goals while upholding the principles upon which Nebula Labs was founded. Although each role has distinct responsibilities, they all contribute to ensuring the success and growth of our organization.',
    people: [
      {
        image: '',
        name: 'Caleb Lim',
  ...
  {
    name: 'Project Leads',
    description: 'Playing a crucial role in shaping every project we work on, project leads orchestrate the transformation of ideas into tangible and functional products. Their coordination skills ensure the successful development of each project, delivering valuable outcomes that positively impact our community.',
    people: [
      {
        image: '',
        name: 'Stephanie Li',
...
const Header = () => {
  ...
  <div className="bg-gradient-to-b from-royal to-white to-75%">
    ...
    <div className="px-8 h-[40vh] flex justify-center items-center">
      <h1 className="text-6xl font-bold text-royal text-center">
        Our Leadership Team
      </h1>
    </div>
  </div>
...
const LeadershipCard = (props: LeadershipCardProps) => {
  return (
    <div className="p-2 flex flex-col items-center grow-0 w-72 gap-4">
      ...
      :
        <Image src={props.image} alt="Headshot" height={280} width={280} className="w-full aspect-square rounded-3xl" />
      }
      ...
      <a href={props.link}>
        <Image src={props.linkImage} alt="Social link" />
      </a>
    </div>
  );
};
...
const LeadershipGroup = (props: LeadershipGroupProps) => {
  ...
  <div className="px-8 lg:px-16 xl:px-32 py-24 flex flex-wrap justify-center gap-8">
    <h2 className="text-5xl font-bold pb-4 text-center">
      {props.name}
    </h2>
    ...
    <div className="flex flex-wrap justify-center gap-16">
      {props.people.map(person => <LeadershipCard {...person} key={person.name} />)}
    </div>
  </div>
)};
...
const Governance = () => (
  <div className="bg-white">
    <Header />
    {governance.map(group => <LeadershipGroup {...group} key={group.name} />)}
